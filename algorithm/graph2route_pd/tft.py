...
class TimeDistributed(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(self.module, nn.Embedding):
            x_reshape = torch.clamp(x_reshape, min=0)
...
class TFT(nn.Module):
    ...
    def forward(self, x_id, x_inputs):
        ...
        for i in range(self.static_variables):
            ...
            emb = self.static_embedding_layers[i](x_id[:, 0, i].long().to(self.device))
            embedding_vectors.append(emb)

        ##Embedding and variable selection
        static_embedding = torch.cat(embedding_vectors, dim=1)
        embeddings_encoder = self.apply_embedding(x_inputs[:, :self.encode_length, :].float().to(self.device),
                                                  static_embedding, apply_masking=False)
        embeddings_decoder = self.apply_embedding(x_inputs[:, self.encode_length:, :].float().to(self.device),
